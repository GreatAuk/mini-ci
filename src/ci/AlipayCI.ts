...
export class AlipayCI extends BaseCI<'mp-alipay'> {
  ...
  async init(): Promise<void> {
    ...
    if (!privateKey) {
      ...
      if (!privateKeyPath || !existsSync(privateKeyPath)) {
        throw new Error(
          `mp-alipay.privateKeyPath 路径不存在：${privateKeyPath || '未配置'}`,
        )
      }
  ...
  async preview() {
    ...
    try {
      ...
      console.log(`预览版二维码已生成，存储在:"${previewQrcodePath}"，二维码内容是："${qrcodeContent}"`)
  ...
  async upload() {
    ...
    try {
      ...
      if (this.config.version && compareVersion(this.config.version, lasterVersion) <= 0) {
        console.warn(
          `上传版本号 "${this.config.version}" 必须大于最新上传版本 "${lasterVersion}"`,
        )
      }
