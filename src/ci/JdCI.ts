...
export class JdCI extends BaseCI<'mp-jd'> {
  ...
  async preview() {
    ...
    try {
      ...
      console.log(`预览二维码已生成，存储在:"${previewQrcodePath}"，二维码内容是："${qrcodeContent}"`)
      ...
      throw new Error(
        `mp-jd preview 执行失败：${error instanceof Error ? error.message : error}`,
      )
    }
  }
  ...
  async upload() {
    ...
    try {
      ...
      await printQrcode2Terminal(qrcodeContent)
      console.log(`体验版二维码已生成，存储在:"${uploadQrcodePath}"，二维码内容是："${qrcodeContent}"`)
      ...
      throw new Error(
        `mp-jd upload 执行失败：${error instanceof Error ? error.message : error}`,
      )
    }
  }
}
